...
const SearchTmdb = ({ mediaType }: { mediaType: MediaType }) => {
  ...
  // Create search parameters with proper enabled control
  ...
  // Use appropriate hook based on media type
  const movieQuery = useSearchTmdbMovie(tmdbMovieParams, {
    enabled: mediaType === MediaType.MOVIE && displaySearchResults,
  });

  const tvQuery = useSearchTmdbTv(tmdbTvParams, {
    enabled: mediaType === MediaType.TV && displaySearchResults,
  });
